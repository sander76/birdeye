...
def app(settings: Settings, input_=None, output=None) -> Application:
    layout = Layout(FileTreeViewer(settings))
    app = Application[str](
        layout=layout,
        full_screen=True,
...
def main(settings: Settings):
    ...
    try:
        _app = app(settings, output=Vt100_Output.from_pty(sys.stderr))
